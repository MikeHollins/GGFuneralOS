        ...
        <div className="flex flex-wrap items-center gap-1 px-2 py-1.5 sm:gap-2 sm:px-3 sm:py-2">
          ...
          <div className="flex flex-wrap items-center gap-1 max-sm:order-3 max-sm:w-full max-sm:flex-nowrap max-sm:overflow-x-auto max-sm:pb-0.5">
            ...
            {(feedMeta?.registers?.length ?? 0) > 0 ? (
              <select
                ...
                className="h-8 w-36 shrink-0 rounded-md border border-neutral-200 bg-neutral-50 px-2 text-xs font-semibold text-neutral-700 outline-none focus:border-[#efb70c] sm:hidden"
          ...
          <div className="ml-auto flex min-w-[190px] items-center justify-end gap-2 max-sm:hidden">
            ...
            <input
              ...
              onChange={(event) => setSearch(event.target.value)}
              placeholder="Search family"
              className="h-8 w-full rounded-md border border-neutral-200 bg-neutral-50 px-2.5 text-xs text-neutral-900 outline-none placeholder:text-neutral-400 focus:border-[#efb70c] focus:ring-2 focus:ring-[#efb70c]/20 sm:w-56"
              aria-label="Search family or deceased"
            />
          </div>
          ...
          {(feedMeta?.registers?.length ?? 0) > 0 ? (
            <select
              value={registerFilter}
              onChange={(event) => chooseRegister(event.target.value)}
              className="h-8 w-full rounded-md border border-neutral-200 bg-neutral-50 px-2 text-xs font-semibold text-neutral-700 outline-none focus:border-[#efb70c] max-sm:hidden lg:hidden"
              aria-label="View a register"
            >
              <option value="">All registers</option>
              {feedMeta!.registers!.map((reg) => (
