...
-- Internal family/NOK contact overrides (absence of a row = source-derived candidate).
CREATE TABLE case_contact_state (
...
CREATE TABLE case_contact_audit (
  id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  case_key        TEXT NOT NULL,
  case_name       TEXT NOT NULL DEFAULT '',
...
CREATE INDEX idx_case_contact_audit_created ON case_contact_audit(created_at DESC);
